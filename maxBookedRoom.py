...
"""
Approach:
   start with empty list, booked_rooms = [].
   iterate through list, for each item, if item[0] == '+',
                  append item[1:] to booked_rooms.
   count the occurance of each element in booked_rooms.
   Return element with highest occurance.
"""

def mostBookedRoom(H):
   booked_rooms = [ room[1:] for room in H if room[0] == '+' ]
   record = {}
   ...
   max_booking = 0
   max_booked_room = ''
   for room, booking in record.items():
      if booking > max_booking:
         max_booking = booking
         max_booked_room += room
      elif booking == max_booking:
         if room < max_booked_room:
            max_booked_room = room
         else:
            pass
      else:
   ...
   return max_booked_room, max_booking

Input =  ["+5A", "+3E", "-5A", "+4F", "+5A", "-3E", "+3E"]

print "max booked room is: " + mostBookedRoom(Input)[0]
print "It was booked " + str(mostBookedRoom(Input)[1]) + " times."
